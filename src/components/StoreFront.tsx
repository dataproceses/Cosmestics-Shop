...
export default function StoreFront({ user, products, categories, onAddToCart, onNavigate, selectedCategory, setSelectedCategory, isShopPage }: StoreFrontProps) {
  ...
  return (
    <div className={`space-y-12 pb-20 ${isShopPage ? 'pt-8' : ''}`}>
      ...
      {/* Featured In Section (Professional Style) */}
      {!isShopPage && (
        <section className="py-12 bg-white border-y border-stone-100">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <p className="text-center text-[10px] font-bold tracking-[0.2em] uppercase text-stone-400 mb-8">As Featured In</p>
            <div className="flex flex-wrap justify-center items-center gap-10 md:gap-20 opacity-50 grayscale hover:grayscale-0 transition-all duration-700">
              <span className="serif text-xl md:text-2xl font-bold tracking-tighter text-stone-900">VOGUE</span>
              <span className="font-serif italic text-xl md:text-2xl text-stone-900">BAZAAR</span>
              <span className="font-sans font-black text-xl md:text-2xl tracking-widest text-stone-900">ELLE</span>
              <div className="flex items-center space-x-1">
                <div className="w-6 h-6 bg-stone-900 rounded-full flex items-center justify-center text-[10px] text-white font-bold">A</div>
                <span className="font-sans font-bold text-lg md:text-xl tracking-tight text-stone-900">allure</span>
              </div>
              <span className="font-sans font-extrabold text-xl md:text-2xl text-stone-900 uppercase tracking-tighter">Glamour</span>
              <span className="serif italic text-xl md:text-2xl text-stone-900">Cosmopolitan</span>
            </div>
          </div>
        </section>
      )}
